...
#define PRVOTNE_BODY 16000
#define DRUHOTNE_BODY 16000 //musí byť dokopy % 8, kvôli AVX, 20024 je % 8
...
//Unit stride
typedef struct body {
    float x[PRVOTNE_BODY+DRUHOTNE_BODY]; //int je viac podporovaný v AVX ako short
    float y[PRVOTNE_BODY+DRUHOTNE_BODY];
} BODY;
...
void compute_squared_distance_matrix_avx(BODY *body, int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < size*8; j += 8) {
            ...
            //_mm256_store_ps(&distance_matrix[i][j], dist2);


            int bitmask = _mm256_movemask_ps(mask2);

            int num_valid = __builtin_popcount(bitmask);

            //if (num_valid) {  // Ak je aspoň jedna platná hodnota
                //for (int k = 0; k < 8; k++) {
                    //if (dist2[k] != 0) {

            if (num_valid) {
                while (bitmask) {
                    int k = __builtin_ctz(bitmask);  // Index prvého nastaveného bitu
                    bitmask &= bitmask - 1;          // Vymaže tento bit

                    if (buffer_idx+1 == current_buffer->size) { //je plný
                        initial_size = initial_size << 1; //*2
                        buffer_idx = 0;

                        CHUNKBUFFER *new = create_buffer(initial_size);
                        printf("new chunk of size: %d\n",initial_size);
                        //current_buffer->children = realloc(current_buffer->children, sizeof(HEAP_CHILD) * initial_size);
                    ...
                    to_change->j = j + k;
                        ...
                        /*if (buffer_idx+1 == current_buffer->size) { //je plný
                            ...
                            //current_buffer->children = new_children;
    ...
    for (int i=0;i<heap_idx;i++) {
        printf("i: %d, j: %d, dist: %g\n",deti[i].i,deti[i].j,deti[i].dist);


    }*/

    //printf("platne hodnoty: %d\n",heap_idx);
